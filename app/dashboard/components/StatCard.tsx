...
export function StatCard({ title, value, trend, icon, color, isNegative = false, className }: StatCardProps) {
  ...
  return (
    <div className={cn(
      "glass-card rounded-[2.5rem] p-6 md:p-10 bg-gradient-to-br border transition-all duration-500 group hover:scale-[1.02] hover:-translate-y-1 relative overflow-hidden", 
      colorMap[color],
      bgGradient[color],
      className
    )}>
       ...
       <div className="flex justify-between items-center mb-8 relative z-10">
          <div className={cn(
            "w-14 h-14 md:w-16 md:h-16 rounded-[1.5rem] flex items-center justify-center shadow-sm transition-all duration-500 group-hover:scale-110 group-hover:rotate-3 border",
          ...
          <div className={cn(
            "flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] md:text-[11px] font-black uppercase tracking-[0.1em] border",
       ...
       <div className="space-y-3 relative z-10">
          <p className="text-[11px] md:text-xs font-black text-muted uppercase tracking-[0.2em] group-hover:opacity-100 transition-all">{title}</p>
          <p className="text-3xl md:text-5xl font-black text-foreground tracking-tighter group-hover:translate-x-1 transition-all duration-500 leading-none">{value}</p>
       </div>
       
       <div className="mt-8 pt-6 border-t border-current opacity-20 relative z-10">
          <p className="text-[10px] md:text-[11px] text-muted font-black uppercase tracking-widest group-hover:opacity-100 transition-opacity flex items-center gap-2">
